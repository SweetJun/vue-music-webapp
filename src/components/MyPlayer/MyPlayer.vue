...
<template>
  <div v-show="playlist.length > 0" class="my-player">
    ...
    <audio ref="audioRef"
           ...
           @ended="ended"
           data-id="343"
           :src="currentSongUrl">Your browser does not support the audio element.</audio>
  </div>
</template>
    ...
    currentSong(newVal, oldVal) {
      ...
      getSongVkey(newVal.mid).then((res) => {
        if (res.code === 0) {
          ...
          // 为防止 修改 vuex state值，使用新的变量作为播放地址播放
          // this.currentSong.url = `${newVal.url}${newVal.mid}.m4a?guid=5931742855&vkey=${vkey}&fromtag=38`
          this.currentSongUrl = `${newVal.url}${newVal.mid}.m4a?guid=5931742855&vkey=${vkey}&fromtag=38`
        }
      })
